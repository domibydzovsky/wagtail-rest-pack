...
export function useSorting(props: SortingProps): SortUse {
    const availableFields = props.sort.fields.filter((field) => field.active(props.pages))
    let defaultField = availableFields.filter((field) => field.isDefault === true)[0]
    if (defaultField === undefined) {
        defaultField = availableFields[0]
    }
    const enabled = props.sort.enabled && props.pages.length > 1 && defaultField !== undefined;
    const [sortField, setSortField] = React.useState({
        fieldName: defaultField ? defaultField.field : undefined,
        directionUp: false
    } as OnChangeProp)
    return {
        props: {...props, sort: {fields: availableFields, enabled}},
        pages: enabled ? props.pages.sort((a, b) => {
            const available = availableFields.filter(field => field.field === sortField.fieldName);
